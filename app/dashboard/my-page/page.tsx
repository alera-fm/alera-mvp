...
export default function MyPageEditor() {
  ...
  const addBlock = (type: string) => {
    ...
    const allowMultiple: string[] = ['video', 'locked_content', 'welcome']
    ...
    switch (type) {
      case 'bio':
        ...
        break
      case 'video':
        setConfig((c: any) => ({ ...c, blocks: [{ id, type: 'video', title: '', video_url: '', thumbnail_url: '' }, ...c.blocks] }))
        break
      case 'release':
      ...
      case 'tip_jar':
        ...
        break
      case 'locked_content':
        setConfig((c: any) => ({ ...c, blocks: [{ id, type: 'locked_content', title: 'Subscribers Only', media_url: '', locked: true, unlock_method: 'subscription' }, ...c.blocks] }))
        break
      case 'welcome':
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    disabled={!(['video','locked_content','welcome'].includes(String(t))) && (config.blocks || []).some((b: any) => b.type === t)}
                    onClick={() => addBlock(String(t))}
                    title={(!( ['video','locked_content','welcome'].includes(String(t)) ) && (config.blocks || []).some((b: any) => b.type === t)) ? 'Already added' : ''}
                  >
                    + {label}
                  </Button>
                ))}
                          ...
                          {b.type === 'video' && (
                            <>
                              ...
                              <div>
                                ...
                                <Input value={b.video_url} onFocus={() => setEditingId(b.id)} onBlur={() => setEditingId(null)} onChange={(e) => setConfig((c: any) => ({ ...c, blocks: c.blocks.map((x: any) => x.id === b.id ? { ...x, video_url: e.target.value } : x) }))} className="alera-form-input" />
                              </div>
                              <div className="md:col-span-2">
                                ...
                                <div className="mt-2 flex items-center gap-3">
                                  <div className="relative h-20 w-20 rounded-lg overflow-hidden bg-white/5 border border-white/10 flex items-center justify-center">
                                    {b.thumbnail_url ? (
                                      <img src={b.thumbnail_url} alt="Thumbnail" className="h-full w-full object-cover" />
                                    ) : (
                                  ...
                                  <div className="space-y-2">
                                    <input
                                      ...
                                      onChange={async (e) => {
                                        ...
                                        if (!file) return
                                        const key = `${b.id}-thumb`
                                        setUploading((u) => ({ ...u, [key]: true }))
                                        try {
                                          const url = await uploadImage(file, 'covers')
                                          setConfig((c: any) => ({ ...c, blocks: c.blocks.map((x: any) => x.id === b.id ? { ...x, thumbnail_url: url } : x) }))
                                        } catch (err) {
                                    ...
                                    />
                                    {uploading[`${b.id}-thumb`] && <span className="text-xs opacity-70">Uploading...</span>}
                                    {b.thumbnail_url && (
                                      <Button type="button" size="sm" variant="outline" onClick={() => setConfig((c:any)=>({ ...c, blocks: c.blocks.map((x:any)=> x.id===b.id ? { ...x, thumbnail_url: '' } : x) }))}>Remove Thumbnail</Button>
                                    )}
                                  </div>
                                </div>
                              </div>
                                    ...
                                    <ul className="list-disc pl-4 space-y-1">
                                      <li>
                                        <button
                                          ...
                                          className="underline"
                                          onClick={() => {
                                            const u = encodeURIComponent(config.artist_name || 'Artist')
                                            const example = `https://www.paypal.com/paypalme/${u}`
                                            setConfig((c:any)=> ({ ...c, blocks: c.blocks.map((x:any)=> x.id===b.id ? { ...x, payment_link: example } : x) }))
                                          }}
                                        >
                                          Use PayPal.me link (paste your handle)
                                        </button>
                                      </li>
                                      <li>
                                      ...
                                      <li>
                                        <a className="underline" href="https://buymeacoffee.com" target="_blank" rel="noreferrer">Use Buy Me a Coffee</a>
                                      </li>
                                      <li>
                                        <a className="underline" href="https://linktr.ee" target="_blank" rel="noreferrer">Linktree tip link</a>
                                      </li>
                                    </ul>
                            ...
                            <>
                              <div className="relative rounded-2xl alera-surface backdrop-blur-md border border-white/10 p-4 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                  ...
                                  <Input value={b.media_url || ''} onFocus={() => setEditingId(b.id)} onBlur={() => setEditingId(null)} onChange={(e) => setConfig((c: any) => ({ ...c, blocks: c.blocks.map((x: any) => x.id === b.id ? { ...x, media_url: e.target.value } : x) }))} className="alera-form-input" />
                                </div>
                                <div className="md:col-span-2">
                                  <Label>Unlock Method</Label>
                                  <Input value={b.unlock_method || ''} onFocus={() => setEditingId(b.id)} onBlur={() => setEditingId(null)} onChange={(e) => setConfig((c: any) => ({ ...c, blocks: c.blocks.map((x: any) => x.id === b.id ? { ...x, unlock_method: e.target.value } : x) }))} className="alera-form-input" />
                                </div>
                              </div>
                            </>
                          )}
