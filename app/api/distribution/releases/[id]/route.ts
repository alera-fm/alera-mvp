...
export async function PUT(
  ...
  try {
    ...
    // Don't allow editing if already under review or published
    if (existingRelease.rows[0].status === 'under_review' || existingRelease.rows[0].status === 'published') {
      return NextResponse.json({ error: 'Cannot edit release that is under review or published' }, { status: 400 })
    }
...
export async function DELETE(
  ...
  try {
    ...
    await pool.query('DELETE FROM releases WHERE id = $1 AND artist_id = $2', [releaseId, authResult.user.id])
