...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get pending earnings (only those with status = 'pending')
    const pendingResult = await pool.query(`
      SELECT COALESCE(SUM(amount_usd), 0) as pending_earnings
      FROM streaming_earnings 
      WHERE artist_id = $1 AND status = 'pending' ${dateFilter}
    `, [artistId])

    // Get earnings by platform from streaming_earnings table (only pending)
    ...
    // Available balance = gross pending earnings - already withdrawn - pending withdrawals
    const availableBalance = grossPendingEarnings - totalWithdrawn - pendingWithdrawals
