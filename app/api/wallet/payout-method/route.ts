...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await pool.query(`
      INSERT INTO payout_methods (artist_id, method, encrypted_account_info, created_at, updated_at)
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await pool.query(`
      SELECT method, encrypted_account_info as account_info, created_at, updated_at
      FROM payout_methods 
      WHERE artist_id = $1
    `, [artistId])
