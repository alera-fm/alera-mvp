  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Invalid status. Must be one of: pending, under_review, sent_to_stores, live, rejected, takedown_requested, takedown",
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      console.log(
        `Admin ${adminId} updated release ${release_id} status to ${status}`
      );
      ...
      if (status === "sent_to_stores") {
        try {
          const { triggerReleaseApprovedEmail } = await import(
            "@/lib/email-automation"
          );
          await triggerReleaseApprovedEmail(
            result.rows[0].artist_id,
            result.rows[0].release_title
          );
        } catch (emailError) {
        ...
        try {
          const { triggerReleaseLiveEmail } = await import(
            "@/lib/email-automation"
          );
          await triggerReleaseLiveEmail(
            result.rows[0].artist_id,
            result.rows[0].release_title
          );
        } catch (emailError) {
        ...
        try {
          const { triggerReleaseRejectedEmail } = await import(
            "@/lib/email-automation"
          );
          await triggerReleaseRejectedEmail(
            result.rows[0].artist_id,
            result.rows[0].release_title
          );
        } catch (emailError) {
