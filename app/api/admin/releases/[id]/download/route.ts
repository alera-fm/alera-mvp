  ...
  try {
    ...
    const releaseResult = await pool.query(`
      SELECT r.*, 
             ...
             u.artist_name as artist_display_name,
             COALESCE(
               json_agg(
                 json_build_object(
                   'id', t.id,
    ...
    if (release.album_cover_url) {
      try {
        ...
        if (coverResponse.ok) {
          ...
          const fileExtension = getFileExtension(release.album_cover_url) || 'jpg'
    ...
    const tracks = Array.isArray(release.tracks) ? release.tracks : JSON.parse(release.tracks || '[]')
      ...
      if (track.audio_file_url) {
        try {
          ...
          if (audioResponse.ok) {
            ...
            const fileName = track.audio_file_name || `track_${track.track_number}.mp3`
            audioFolder?.file(fileName, audioBuffer)
          }
        } catch (error) {
          console.error(`Error downloading audio file for track ${track.track_number}:`, error)
    ...
    // Return the zip file
    const fileName = `${release.release_title.replace(/[^a-zA-Z0-9]/g, '_')}_complete_release_data.zip`
    ...
    return NextResponse.json({ error: 'Failed to download release data' }, { status: 500 })
  }
}
...
${tracks.map((track: any) => 
  `- Track ${track.track_number}: ${track.track_title} (${track.audio_file_name || 'No audio file'})`
).join('\n')}
...
DISTRIBUTION STORES:
${Array.isArray(release.selected_stores) ? release.selected_stores.join(', ') : JSON.parse(release.selected_stores || '[]').join(', ') || 'None selected'}
