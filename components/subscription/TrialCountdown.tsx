...
export function TrialCountdown() {
  ...
  const styles = {
    container:
      "bg-gradient-to-r from-purple-50 to-yellow-50 dark:from-purple-900/30 dark:to-yellow-800/20 border-2 border-purple-300 dark:border-purple-700 shadow-lg",
    text: "text-purple-900 dark:text-purple-100 font-bold",
    subtext: "text-purple-800 dark:text-purple-200",
    button:
      "bg-gradient-to-r from-purple-600 to-yellow-500 hover:from-purple-700 hover:to-yellow-600 text-white font-semibold shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-200",
  };
  ...
  const getSubMessage = () => {
    return "Upgrade to Plus or Pro to unlock unlimited releases, advanced features, and keep your music live forever.";
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <h3
                className={`text-sm md:text-base font-semibold leading-tight ${styles.text}`}
              >
                {getMessage()}
              </h3>
              <p
                className={`text-xs md:text-sm mt-1 leading-tight ${styles.subtext} hidden sm:block`}
              >
                {getSubMessage()}
              </p>
            </div>
...
export function TrialCountdownCompact() {
  const { subscription, showUpgradeDialog } = useSubscription();

  if (subscription?.tier !== "trial") return null;

  const handleClick = () => {
    showUpgradeDialog(
      "Upgrade to continue using all features and create unlimited releases.",
      "plus"
    );
  };
