  ...
  const handleWithdrawalRequest = async () => {
    ...
    if (payoutMethod?.status === 'rejected') {
      toast({
        ...
        description: "Your payout method has been rejected. Please add a new valid method before requesting withdrawals.",
        variant: "destructive"
      })
      return
    }
    ...
    if (payoutMethod?.status === 'pending') {
      toast({
        ...
        description: "Your payout method is pending approval. Please wait for admin approval before requesting withdrawals.",
        variant: "destructive"
      })
      return
    }
                ...
                {payoutMethod.status === 'rejected' && (
                  <div className="mt-3 p-3 bg-red-100 dark:bg-red-900/30 rounded-lg">
                    ...
                    <p className="text-xs text-red-700 dark:text-red-500 mt-1">
                      Please add a new, valid payout method to continue receiving payments. Contact support if you need assistance.
                    </p>
                  </div>
                )}
                ...
                {payoutMethod.status === 'pending' && (
                  <div className="mt-3 p-3 bg-yellow-100 dark:bg-yellow-900/30 rounded-lg">
                    ...
                    <p className="text-xs text-yellow-700 dark:text-yellow-500 mt-1">
                      We're reviewing your payout method. You'll be able to request withdrawals once it's approved.
                    </p>
                  </div>
                )}
              </div>
