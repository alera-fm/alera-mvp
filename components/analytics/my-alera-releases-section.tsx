...
export default function MyAleraReleasesSection() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const processedData = {
          ...
          streamsByPlatform:
            data.streamsByPlatform?.map((item: any) => ({
              ...item,
          ...
          dailyStreams:
            data.dailyStreams?.map((item: any) => ({
              ...item,
              streams: parseInt(item.streams) || 0,
            })) || [],
          shazamRecognitions: parseInt(data.shazamRecognitions) || 0,
          shazamCountries:
            data.shazamCountries?.map((item: any) => ({
              ...item,
              count: parseInt(item.count) || 0,
            })) || [],
          shazamCities:
            data.shazamCities?.map((item: any) => ({
              ...item,
              count: parseInt(item.count) || 0,
            })) || [],
          ...
          tiktokTerritories:
            data.tiktokTerritories?.map((item: any) => ({
              ...item,
              views: parseInt(item.views) || 0,
            })) || [],
          metaViews: parseInt(data.metaViews) || 0,
          metaTerritories:
            data.metaTerritories?.map((item: any) => ({
              ...item,
  ...
  return (
    <div className="space-y-6">
      ...
      {analyticsData?.trialStatus?.isTrial && (
          ...
          <div className="flex items-start">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-amber-700 dark:text-amber-300">
                <p>
                  {analyticsData.trialStatus.isLimited
                    ? "You're viewing basic analytics. Upgrade to Plus or Pro to access detailed analytics including geographic data and social media performance."
                    : "You're viewing trial analytics. After using your free release, analytics will be limited. Upgrade to Plus or Pro to maintain full access."}
                </p>
                {analyticsData.trialStatus.dataDelayed && (
                  <p className="mt-1">
                    Note: Trial analytics are updated weekly rather than daily.
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">Total Shazams</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {formatNumber(analyticsData.tiktokViews)}
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {analyticsData.dailyStreams.length > 1 ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-[300px] text-center">
                    ...
                    <div className="text-sm text-muted-foreground">
                      Streams on{" "}
                      {formatDate(analyticsData.dailyStreams[0].reporting_date)}
                    </div>
                    ...
                    <PieChart>
                      <Pie
                        ...
                        label={({ platform, percent }) =>
                          `${platform} ${((percent || 0) * 100).toFixed(1)}%`
                        }
                        outerRadius={80}
                        fill="#8884d8"
                        dataKey="streams"
                    ...
                    <PieChart>
                      <Pie
                        ...
                        label={({ country, percent }) =>
                          `${country} ${((percent || 0) * 100).toFixed(1)}%`
                        }
                        outerRadius={80}
                        fill="#8884d8"
                        dataKey="streams"
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Streams by device type</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    <PieChart>
                      <Pie
                        ...
                        label={({ device_type, percent }) =>
                          `${device_type} ${((percent || 0) * 100).toFixed(1)}%`
                        }
                        outerRadius={80}
                        fill="#8884d8"
                        dataKey="streams"
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    {analyticsData.tiktokTerritories &&
                    analyticsData.tiktokTerritories.length > 0 ? (
                      <ResponsiveContainer width="100%" height={300}>
                        <PieChart>
                          <Pie
                            ...
                            cy="50%"
                            labelLine={false}
                            label={({ territory, percent }) =>
                              `${territory} ${((percent || 0) * 100).toFixed(
                                0
                              )}%`
                            }
                            outerRadius={80}
                            ...
                            dataKey="views"
                          >
                            {analyticsData.tiktokTerritories.map(
                              (entry, index) => (
                                <Cell
                                  key={`cell-${index}`}
                                  fill={
                                    entry.territory === "Other"
                                      ? "#999999"
                                      : [
                                          "#8884d8",
                                          "#82ca9d",
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    {analyticsData.metaTerritories &&
                    analyticsData.metaTerritories.length > 0 ? (
                      <ResponsiveContainer width="100%" height={300}>
                        <PieChart>
                          <Pie
                            ...
                            dataKey="events"
                          >
                            {analyticsData.metaTerritories.map(
                              (entry, index) => (
                                <Cell
                                  key={`cell-${index}`}
                                  fill={
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    {analyticsData.shazamCountries &&
                    analyticsData.shazamCountries.length > 0 ? (
                      <ResponsiveContainer width="100%" height={300}>
                        <PieChart>
                          <Pie
                            ...
                            labelLine={false}
                            label={({ country, percent }) =>
                              `${country} ${((percent || 0) * 100).toFixed(0)}%`
                            }
                            outerRadius={80}
                            fill="#8884d8"
                            dataKey="count"
                          >
                            {analyticsData.shazamCountries.map(
                              (entry, index) => (
                                <Cell
                                  key={`cell-${index}`}
                                  fill={
                ...
                <Card>
                  ...
                  <CardContent>
                    {analyticsData.shazamCities &&
                    analyticsData.shazamCities.length > 0 ? (
                      <ResponsiveContainer width="100%" height={300}>
                        <PieChart>
                          <Pie
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground text-center max-w-md">
              Connect with admin if you think something is missing. Analytics
